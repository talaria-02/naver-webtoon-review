...
function demoFilterWebtoons($webtoons, $q = '', $day = '', $sort = 'rating') {
    $filtered = array_filter($webtoons, function ($w) use ($q, $day) {
        $posTitle = function_exists('mb_stripos') ? mb_stripos($w['title'], $q) : stripos($w['title'], $q);
        $posAuthors = function_exists('mb_stripos') ? mb_stripos($w['authors'], $q) : stripos($w['authors'], $q);
        if ($q !== '' && $posTitle === false && $posAuthors === false) return false;
